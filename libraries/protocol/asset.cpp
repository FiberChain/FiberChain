...
namespace fiberchain { namespace protocol {
      ...
      void asset::change_fraction_size(uint8_t size)
      {
         share_type old_precision = precision();
         set_decimals(size);
         share_type new_precision = precision();

         if(new_precision > old_precision) {
            share_type multiply_rate = new_precision / old_precision;
            amount *= multiply_rate;
         } else {
            share_type divide_rate = old_precision / new_precision;
            amount /= divide_rate;
         }
      }
