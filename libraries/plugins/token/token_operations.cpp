...
namespace fiberchain { namespace token {
   ...
   void create_token_operation::validate()const
   {
      try {
         ...
         FC_ASSERT( init_supply_amount <= FIBERCHAIN_TOKEN_MAX, "Initial supply amount is over max (90,000,000,000)." );
   ...
   void set_token_staking_interest_operation::validate()const
   {
      try {
         ...
         if(percent_interest_rate.find(".") != std::string::npos ) {
            ...
            FC_ASSERT( lower_decimal >= 0 && lower_decimal <= FIBERCHAIN_STAKING_INTEREST_PRECISION_DIGITS
