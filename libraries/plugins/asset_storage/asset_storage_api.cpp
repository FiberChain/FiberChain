...
namespace futurepia { namespace asset_storage {
   ...
   namespace detail {
      class dapp_api_impl
      {
         public:
            ...
            vector< asset_storage_api_object > lookup_dapps( string& lower_bound_name, uint32_t limit ) const;
            optional< asset_storage_api_object > get_dapp( string dapp_name ) const;
            vector< asset_storage_api_object > get_dapps_by_owner( string& owner ) const;
      ...
      vector< asset_storage_api_object > dapp_api_impl::lookup_dapps( string& lower_bound_name, uint32_t limit ) const
      {
         vector< asset_storage_api_object > results;
      ...
      optional< asset_storage_api_object > dapp_api_impl::get_dapp( string dapp_name ) const
      {
         const auto& dapp_idx = _app.chain_database()->get_index< dapp_index >().indices().get< by_name >();
         auto itr = dapp_idx.find( dapp_name );
         if( itr != dapp_idx.end() )
            return asset_storage_api_object( *itr );
         else 
            return {};
      }

      vector< asset_storage_api_object > dapp_api_impl::get_dapps_by_owner( string& owner ) const
      {
         vector < asset_storage_api_object > results;
   ...
   vector< asset_storage_api_object > dapp_api::lookup_dapps( string lower_bound_name, uint32_t limit ) const
   {
   ...
   optional< asset_storage_api_object > dapp_api::get_dapp( string dapp_name ) const
   {
      return _my->database().with_read_lock( [ & ]()
      {
   ...
   vector< asset_storage_api_object > dapp_api::get_dapps_by_owner( string owner ) const
   {
      return _my->database().with_read_lock( [ & ]()
      {
