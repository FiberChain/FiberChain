...
)
{
   ...
   while( produced < count )
   {
      ...
      if( debug_key != "" )
      {
         ...
         {
            if( logging ) wlog( "Modified key for bobserver ${w}", ("w", scheduled_bobserver_name) );
            debug_update( [=]( chain::database& db )
            {
               db.modify( db.get_bobserver( scheduled_bobserver_name ), [&]( chain::bobserver_object& w )
               {
      ...
      else
      {
         ...
         if( key_storage != nullptr )
            key_storage->maybe_get_private_key( debug_private_key, scheduled_key, scheduled_bobserver_name );
         if( !debug_private_key.valid() )
         {
            if( logging ) elog( "Skipping ${wit} because I don't know the private key", ("wit", scheduled_bobserver_name) );
            new_slot = slot+1;
            FC_ASSERT( slot < miss_blocks+50 );
         }
      }
      db.generate_block( scheduled_time, scheduled_bobserver_name, *debug_private_key, skip );
      ++produced;
      slot = new_slot;
   }
